...
import com.evans.playnow.helpers.Event
import com.evans.playnow.helpers.Resource
...
class MusicServiceConnection(
    ...
    private val mediaConnectionCallback = MediaBrowserConnectionCallback(context)
    ...
    private inner class MediaBrowserConnectionCallback(
        ...
        override fun onConnectionFailed() {
            _isConnected.postValue(
                Event(Resource.error("Couldn't connect to media browser", false))
            )
        }
    }
        ...
        override fun onSessionEvent(event: String?, extras: Bundle?) {
            ...
            when (event) {
                NETWORK_ERROR -> _networkError.postValue(
                    Event(Resource.error("Please check you connection", null))
                )
            }
        }
